...
export const confirmUser = createEffect<
  string,
  R<{ email: string }>,
  E<{ code: string }>
>(async (code) => {
...
ConfirmationCode.on(UPDATE_CONFIRMATION_CODE, (store, code) => ({
  code,
  status: 'set',
}))
...
sample({
  ...
  filter: (store: NULL<ConfirmationCode>): store is ConfirmationCode =>
    store?.status === 'set',
  fn: (store) => store.code,
  target: confirmUser,
});

...
export const CHANGE_FORM_PROPERTY = createEvent<Partial<{[property in keyof SignInState['form']]: string}>>()
...
export const SignInStore = createStore<SignInState>({
  ...
  error: undefined,
  form: {
    username: 'gena.razmakhnin@gmail.com',
    password: '12345'
  }
})
...
ConfirmationEmail
  .on(confirmUser.doneData, (store, { response }) => response.data.email)

ConfirmationCode
  .on(UPDATE_CONFIRMATION_CODE, (store, code) => ({ code, status: 'set' }))
...
SignInStore
  ...
  .on(signIn.fail, (store, { error }) => ({ ...store, error: error.response.data }))

...
export const endpoints = [
  {
    ...
    handler: async (
      request: FastifyRequest<{ Body: { data: Input<typeof signupSchema> } }>,
      reply: FastifyReply,
    ) => {
      console.log('HERE');
      const {
      ...
      if (userSearch.entry?.length) {
        return reply.status(400).send({ code: 'EMAIL_TAKEN' });
      }
  ...
  },
  {
    ...
    authRequired: false,
    handler: async (
      request: FastifyRequest<{ Body: { data: { code: string } } }>,
      reply: FastifyReply,
    ) => {
      const {
        aidboxClient,
  ...
  },
  {
    ...
    handler: async (
      ...
      if (!userRaw.entry?.length) {
        return reply.status(404).send({ code: 'EMAIL_NOT_FOUND' });
      }
